...
6. **大文件优化**：针对大文件（>50GB）自动应用优化配置，包括更大的磁盘缓存、更多连接数、自动恢复等
...
Downloader 类针对大文件（>50GB）自动应用以下优化：

**自动检测和优化：**
- 自动检测文件大小，对于超过 50GB 的文件自动应用优化配置
- 使用手动下载模式，避免在文件检查时被自动暂停
- 自动监控下载状态，如果检测到暂停会自动恢复

**性能优化配置：**
- **磁盘缓存**：从默认 32MB 增加到 512MB，提高大文件下载性能
- **磁盘写入队列**：设置为 1GB，减少 IO 阻塞
- **连接数**：每个 torrent 最大连接数设置为 200（小文件为 50）
- **文件优先级**：所有文件设置为最高优先级（7）
- **自动恢复**：在下载循环中自动检测并恢复被暂停的下载
...
// 大文件会自动应用优化
if (downloader.start_download("large_file.torrent", "./downloads"))
{
    // 大文件下载会自动使用优化配置
    // 无需额外设置
    while (downloader.is_downloading() && !downloader.is_finished())
...
**注意事项：**
- 大文件下载可能需要较长时间，请确保有足够的磁盘空间
- 下载过程中会自动监控状态，无需手动干预
- 如果下载被暂停，系统会自动恢复（除非手动暂停）
...
**重要提示：**
1. **磁盘空间**：确保有足够的磁盘空间（至少是文件大小的 1.1 倍）
2. **下载时间**：大文件下载可能需要很长时间，请耐心等待
3. **自动优化**：系统会自动检测大文件并应用优化配置，无需手动设置
4. **状态监控**：系统会自动监控下载状态，如果检测到暂停会自动恢复
5. **网络连接**：保持网络连接稳定，支持断点续传

**性能建议：**
- 使用 SSD 可以提高大文件下载性能
- 确保网络带宽充足
- 如果有多个 tracker，可以提高连接成功率
...
**大文件优化配置：**
- 磁盘缓存大小：512MB（默认 32MB）
- 磁盘缓存过期时间：300 毫秒
- 磁盘写入队列：1GB
- 每个 torrent 最大连接数：200（大文件）/ 50（小文件）
